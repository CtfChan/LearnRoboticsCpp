...
std::pair<std::vector<int>, std::vector<int>> AStar::plan(float sx, float sy, float gx, float gy, Gnuplot& gp) {
...
std::pair<std::vector<int>, std::vector<int>> AStar::calculateFinalPath(Node goal_node, 
                                            std::unordered_map<int, Node>& closed_set) {
    // for plotting   
    std::vector<int> rx;
    std::vector<int> ry;  
...
void AStar::calculateObstacleMap( std::vector<int>& ox,  std::vector<int>& oy) {
...
int main() {
    ...
    auto [rx, ry] = astar.plan(sx, sy, gx, gy, gp);
