...
//     return si->satisfiesBounds(s) && (x < 5 || x > 13 || (y > 8.5 && y
//     < 9.5));
// }
...
// void printTrajectory(const ob::StateSpacePtr &space,
...
//     // print the distance for (x,y,theta) for all points in a 3D grid in
//     SE(2)
...
//                 to[2] = boost::math::constants::pi<double>() *
//                         (2. * (double)k / num_pts - 1.);
...
//             "print trajectory from (0,0,0) to a user-specified x, y, and "
//             "theta")("distance", "print distance grid");
...
//         if (vm.count("easyplan") != 0u) plan(space, true);
//         if (vm.count("hardplan") != 0u) plan(space, false);
//         if (vm.count("trajectory") != 0u)
//             printTrajectory(space, vm["trajectory"].as<std::vector<double>
//             >());
//         if (vm.count("distance") != 0u) printDistanceGrid(space);
...
// use print trajectory function to illustrate sample path from A -> B
// plot with gnu
